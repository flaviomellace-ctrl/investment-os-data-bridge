...
import csv, gzip, hashlib, json, math, os, time, urllib.request, zlib
...
def decode_http_body(raw:bytes,content_encoding:str)->bytes:
    """Decode SEC HTTP content encoding before UTF-8 JSON parsing."""
...
def fetch_companyfacts(cik:str,email:str,last_request:[float])->dict:
    """Fetch SEC Companyfacts with fair-access spacing and transient retries."""
    url=companyfacts_url(cik)
    last_exc=None
    for attempt in range(3):
        elapsed=time.monotonic()-last_request[0]
        if elapsed<SEC_MIN_SECONDS:
            time.sleep(SEC_MIN_SECONDS-elapsed)
        ...
        try:
            ...
            body=decode_http_body(raw,encoding)
...
"""
    ...
    if fetch_fail:
        print("R10B INCOMPLETE: fetch failures remain; candidate must not advance to R10C.")
        return 2
    return 0
